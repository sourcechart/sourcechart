...
import type { StabilityDict, SingleLinkage } from './types';
...
export function selectClustersUsingEOM(
	...
	for (const node of nodeList) {
		const childSelection = clusterTree.filter((c) => c.parent === node).map((c) => c.child);
		const subtreeStability = childSelection
			.map((cs) => stability.get(cs) || 0)
			.reduce((r, n) => r + n, 0);
		if (
			subtreeStability > (stability.get(node) || 0) ||
			(clusterSizes.get(node) || 0) > maxClusterSize
		) {
	...
	if (clusterSelectionEpsilon !== 0 && clusterTree.length > 0) {
		...
		if (eomClusters.size == 1 && [...eomClusters.keys()][0] == root) {
			...
			selectedClusters = epsilonSearch(
				eomClusters,
				clusterTree,
