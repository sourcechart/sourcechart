...
export function computeStabilities(condensedTree: SingleLinkage) {
	...
	const sortedChildData = condensedTree.map((c) => [c.child, c.lambda]).sort((a, b) => a[0] - b[0]);
...
export function getClusterNodes(
	...
	const nonClusterChilds = condensedTree.filter((t) => t.size === 1).map((t) => t.child);
	...
	const clusterNodes = [...isCluster.entries()]
		.filter((e) => e[1])
		.map((e) => e[0])
		.sort();
			...
			if (clusterNodes.length === 1 && allowSingleCluster) {
				...
				if (clusterSelectionEpsilon !== 0.0) {
					...
					const parentMatches = condensedTree
						...
						.map((t) => t.lambda);
					if (childMatch && parentMatches && childMatch.lambda >= Math.max(...parentMatches)) {
						result[n] = clusterLabelMap.get(cluster);
					} else {
						result[n] = -1;
					}
