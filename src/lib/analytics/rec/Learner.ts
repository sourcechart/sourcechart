...
export class Learner {
	...
	constructor(dataBlock?: DataBlock, options?: optionsLearner) {
		if (dataBlock != null) {
			...
			this.MFC = new MatrixFactorization(
				this.usersNum,
				this.itemsNum,
				this.embeddingOutputSize,
				this.l2Labmda,
				this.ratingRange
			);
			...
			this.dataBlock.datasetInfo.userToModelMap.forEach((value, key) =>
				this.modelToUserMap.set(value, key)
			);
			this.dataBlock.datasetInfo.itemToModelMap.forEach((value, key) =>
				this.modelToItemMap.set(value, key)
			);
		} else {
		...
		switch (optimizerName) {
			...
			default:
				throw new ValueError(
					`${optimizerName} optimzer does not exist in pprec. Only adam, sgd, and rmsprop are supported (lower case)`
				);
		}
		this.model.compile({
    ...
    */
	async recommendItems(userId: any, k: number, alreadyWatched: boolean = false): Promise<number[]> {
		if (k > this.itemsNum)
			throw new ValueError(
				`The value k = ${k} is bigger than the number of items you actually have.`
			);
    ...
    */
	newUser(userId: any) {
		...
		if (this.usersNum > 1) {
			...
			userEmbeddingWeight = tf.concat([
				userEmbeddingWeight,
				userEmbeddingWeight.mean(0).reshape([1, this.embeddingOutputSize])
			]);
			this.MFC = new MatrixFactorization(
				this.usersNum,
				this.itemsNum == 0 ? 1 : this.itemsNum,
				this.embeddingOutputSize,
				0,
				this.ratingRange,
				[userEmbeddingWeight]
    ...
    */
	newItem(itemId: any) {
		...
		if (this.itemsNum > 1) {
			...
			itemEmbeddingWeight = tf.concat([
				itemEmbeddingWeight,
				itemEmbeddingWeight.mean(0).reshape([1, this.embeddingOutputSize])
			]);
			this.MFC = new MatrixFactorization(
				this.usersNum == 0 ? 1 : this.usersNum,
				this.itemsNum,
				this.embeddingOutputSize,
				0,
				this.ratingRange,
    ...
    */
	mostSimilarUsers(id: any, k = 10): string[] {
		...
		if (k > this.usersNum)
			throw new ValueError(
				`The value k = ${k} is bigger than the number of users you actually have.`
			);
		...
		return indicesArray
			.filter((e: number) => e !== mappedId)
			.map((e) => this.modelToUserMap.get(e));
	}
    ...
    */
	mostSimilarItems(id: any, k = 10): number[] {
		...
		return indicesArray
			.filter((e: number) => e !== mappedId)
			.map((e) => this.modelToItemMap.get(e));
	}
    ...
    */
	save(path: string): Promise<io.SaveResult> {
		...
		let userMap = JSON.stringify(
			Array.from((this.dataBlock.datasetInfo.userToModelMap as Map<any, number>).entries())
		);
		fs.writeFileSync(`${path}/userToModelMap.txt`, userMap);

		let itemMap = JSON.stringify(
			Array.from((this.dataBlock.datasetInfo.itemToModelMap as Map<any, number>).entries())
		);
	...
	async load(path: string): Promise<Learner> {
		...
		this.dataBlock.datasetInfo = {
			...
			userToModelMap: new Map(JSON.parse(loadedUserMap)),
			itemToModelMap: new Map(JSON.parse(loadedItemMap))
		};
		this.dataBlock.datasetInfo.userToModelMap.forEach((value, key) =>
			this.modelToUserMap.set(value, key)
		);
