...
export class MatrixFactorization {
	userInputLayer;
	userEmbeddingLayer: Embedding;
	userEmbeddingLayerOutput;
	itemInputLayer;
	itemEmbeddingLayer;
	itemEmbeddingLayerOutput;
	dotLayer;
	...
	constructor(
		...
		weightDecay: number,
		ratingRange?: number[],
		userEmbeddingWeights?,
		itemEmbeddingWeights?
	) {
		...
		this.userEmbeddingLayerOutput = tf.layers
			.flatten({ name: 'flat1' })
			.apply(this.userEmbeddingLayer.apply(this.userInputLayer));
		this.itemEmbeddingLayerOutput = tf.layers
			.flatten({ name: 'flat2' })
			.apply(this.itemEmbeddingLayer.apply(this.itemInputLayer));

		// if user did not specify a range for the ratings
		if (ratingRange == null) {
			this.dotLayer = tf.layers
				.dot({ axes: -1, name: 'rating' })
				.apply([this.userEmbeddingLayerOutput, this.itemEmbeddingLayerOutput]);
			this.model = tf.model({
				inputs: [this.userInputLayer, this.itemInputLayer],
				outputs: this.dotLayer
			});
		}
		...
		// if user did specify a range for the ratings high and low
			...
			this.dotLayer = tf.layers
				.dot({ axes: -1, name: 'dot' })
				.apply([this.userEmbeddingLayerOutput, this.itemEmbeddingLayerOutput]);
			this.sigmoidLayer = new SigmoidRange({
				high: ratingRange[1],
				low: ratingRange[0],
				name: 'rating'
			}).apply(this.dotLayer);
