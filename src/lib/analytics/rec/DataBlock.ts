...
interface IdatasetInfo {
	...
	usersNum: number;
	itemsNum: number;
	userToModelMap: Map<any, number>;
	itemToModelMap: Map<any, number>;
}
...
export class DataBlock {
	...
	constructor(redisUrl?: string) {
		this.datasetInfo = {
			...
			usersNum: 0,
			itemsNum: 0,
			userToModelMap: new Map(),
			itemToModelMap: new Map()
		};
    ...
    */
	async fromCsv(path: string, options: optionsDataBlockCsv) {
		...
		this.datasetInfo = await this.getInfoOnCsv(
			path,
			options.userColumn,
			options.itemColumn,
			options.delimiter
		...
		let csvDataset: tf.data.Dataset<any> = tf.data
			.csv(myPath, {
			...
			.map((x: any) => ({
				xs: {
					user: this.datasetInfo.userToModelMap.get(`${x.xs[options.userColumn]}`),
					item: this.datasetInfo.itemToModelMap.get(`${x.xs[options.itemColumn]}`)
				},
				ys: { rating: Number(x.ys[options.ratingColumn]) }
			}));

		//split the dataset into train and valid set
		let validationPercentage =
			options?.validationPercentage == null ? 0.1 : options?.validationPercentage;
		...
		this.trainingDataset = this.trainingDataset.map((x: Idataset) => ({
			xs: { user: x.xs.user.reshape([-1, 1]), item: x.xs.item.reshape([-1, 1]) },
			ys: { rating: x.ys.rating.reshape([-1, 1]) }
		}));

		if (validationPercentage > 0) {
			this.validationDataset = csvDataset.skip(trainSize).batch(this.batchSize);
			this.validationDataset = this.validationDataset.map((x: Idataset) => ({
				xs: { user: x.xs.user.reshape([-1, 1]), item: x.xs.item.reshape([-1, 1]) },
				ys: { rating: x.ys.rating.reshape([-1, 1]) }
			}));
		}
	...
	async fromArray(
		...
		if (validationPercentage > 0) {
			...
			for (let i = 0; i < items.length; i++) {
				psuedoTrainingDataset.push({
					xs: { user: users[i], item: items[i] },
					ys: { rating: ratings[i] }
				});
			}
    ...
    */
	getInfoOnCsv(
		...
		let datasetInfo_ = new Promise<IdatasetInfo>(function (resolve, reject) {
			...
			csv
				.parseFile(path, { headers: true, delimiter: delimiter })
    ...
    */
	splitTrainValidTensor(
		...
		for (let i = 0; i < trainingRatings.length; i++) {
			psuedoTrainingDataset.push({
				xs: { user: trainingUsers[i], item: trainingItems[i] },
				ys: { rating: trainingRatings[i] }
			});
		...
		for (let i = 0; i < validationRatings.length; i++) {
			psuedoValidationDataset.push({
				xs: { user: validationUsers[i], item: validationItems[i] },
				ys: { rating: validationRatings[i] }
			});
    ...
    */
	async save(outputFile: string, validationFileName?: string): Promise<void> {
		...
		await this.trainingDataset.forEachAsync(function (e: Idataset) {
			let users_ = e.xs.user.dataSync();
			let items_ = e.xs.item.dataSync();
			let ratings_ = e.ys.rating.dataSync();
			for (let i = 0; i < ratings_.length; i++) stream.write([users_[i], items_[i], ratings_[i]]);
		});
		...
		if (this.validationDataset != null)
			await this.validationDataset.forEachAsync(function (e: Idataset) {
				let users_ = e.xs.user.dataSync();
				let items_ = e.xs.item.dataSync();
				let ratings_ = e.ys.rating.dataSync();
				for (let i = 0; i < ratings_.length; i++) stream.write([users_[i], items_[i], ratings_[i]]);
    ...
    */
	...
	async redisConfig(url) {
		if (url == null) this.client = createClient();
		else
