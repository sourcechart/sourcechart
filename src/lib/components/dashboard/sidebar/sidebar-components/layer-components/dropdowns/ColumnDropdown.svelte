<script lang="ts">
	...
	export let dropdownType: DropdownType;
	...
	$: if (
		...
		if (dropdownType === 'xColumn') currentValue = $allCharts[$i].xColumn;
		if (dropdownType === 'yColumn') currentValue = $allCharts[$i].yColumn;

		// Check if xColumn value exists in schema's name
		let columnExistsinSchema = $allCharts[$i].schema.some(
...
<div bind:this={container} class="flex-grow relative w-full">
	<div class="flex items-center justify-between">
		<button
			...
			on:blur={() => null}
		>
			<span class="text-sm text-neutral-300 ml-1">Longitude</span>
